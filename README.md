...
![Live deployed page: a Mac OS 7/8-style IDE with a top menubar (Apple / File / Edit / View / Special / Windows / Help, with a "cv-mac <hash>" build stamp and clock on the right), and several draggable WinBox panes. Project picker on the left. Playground in the centre showing the wasm-gworld sample with syntax-highlighted C source — case labels and keywords in navy, comments in muted brown, types and numbers picked out in colour. To the right of the editor a pinned "Toolbox Reference — NewGWorld" window shows the Inside-Macintosh-style signature, a one-paragraph description, and a clickable "See also" list (DisposeGWorld, SetGWorld, GetGWorldPixMap, LockPixels) — opened by ⌘-clicking NewGWorld in the editor (or via Help → Toolbox Reference…). Top-right: Macintosh pane running System 7.5.5 in BasiliskII with the bundled apps from Startup Items. Bottom-right: Output panel with a Build Log tab capturing per-stage timings.](public/screenshot-deployed.png)
