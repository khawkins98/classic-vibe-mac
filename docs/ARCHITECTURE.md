...
browser tab. No backend. No relay. No auth. The site ships
HTML + JS + WASM + a chunked HFS disk image; every byte after that
runs in the user's tab.
...
3. **Worker spawn.** `new Worker(new URL('./emulator-worker.ts',
   import.meta.url), { type: 'module' })`. The loader posts an init
   message with manifest URL, ROM URL, screen + RAM config.
...
`FSWrite` to `:Unix:foo`; the host page can `await
worker.fsReadFile('/Shared/foo')` (or read it back via the worker's
postMessage bridge). This is the channel the Pixel Pad demo (#17)
will use to export drawings.
...
- has its own four-letter creator code (Reader=`CVMR`,
  MacWeather=`CVMW`) — passed to `add_application(... CREATOR XXXX ...)`
  so the `-c` flag reaches Rez and the MacBinary header carries
  `APPL/XXXX`. Without the creator, Finder binding silently no-ops
...
| API | What for | Notes |
|-----|----------|-------|
| `SharedArrayBuffer` | video framebuffer + input ring shared with worker | Hard requirement. Needs cross-origin isolation. |
| `Atomics.wait` / `Atomics.notify` | the four-state input lock between UI and worker | Cyclical lock; the layout has to match Infinite Mac's `InputBufferAddresses` byte-for-byte. |
