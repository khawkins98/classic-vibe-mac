...
**Context:** Glypha III (#255) silent-exited on launch (#256, closed 2026-05-18) — at least three independent failure modes stacked on top of each other (the ToolTrap shim semantic bug, the missing PICT resources, eventually the missing 'snd ' resources). The user-visible symptom was identical for all of them: app vanishes within a second, no dialog, no message anywhere. Debugging blind is awful. The final root cause turned out to be a wasm-rez C-stack overflow on large resource forks (#287); the recipe for getting from "vendored app crashes silently" to "vendored app runs" is now distilled in [`docs/DEBUGGING-VENDORED-APPS.md`](docs/DEBUGGING-VENDORED-APPS.md) (when it breaks) and its happy-path counterpart [`docs/VENDORING-A-MAC-APP.md`](docs/VENDORING-A-MAC-APP.md).
