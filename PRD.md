...
| **Structural-pass-but-runtime-fail blind spot** (resolved 2026-05-15) | A wasm-built `.bin` could pass every structural check (valid MacBinary II, valid resource fork) and still silently exit at app launch on the deployed BasiliskII — the deploy-and-eyes-on cycle was 15-30 min per iteration. `tools/m68k-runner/` (closes #89) is a native Musashi-based boot tracer that runs the same binary through a 68k CPU emulator with instruction-by-instruction trace + A-line trap logging. MVP took ~90 minutes; diagnosed the next bug in 30 seconds. Now the backbone of toolchain testing — see LEARNINGS Key Story #1. |
| **wasm-rez C-stack overflow on large resource forks** (resolved 2026-05-17) | wasm-rez's recursive evaluator overflowed Emscripten's 64KB default `STACK_SIZE` on resources with > ~1340 `$"…"` hex literals (left-leaning CONCAT trees in the AST). Cliff was inside Glypha's `.r`; the failure surfaced as a misleading Bison type-id assertion or `memory access out of bounds`. Fixed in #287 by bumping `STACK_SIZE` to 8 MB in `tools/wasm-rez/CMakeLists.txt` — cost is virtual address space, not physical pages. Same fix unblocked Glypha's full upstream 2.7 MB `.r` (compiles in ~230 ms now). See LEARNINGS Key Story #10 for the broader "premise was the bug" arc. |
| **Malformed `ALRT` shorthand silently broke samples** (resolved 2026-05-18) | The shorthand `resource 'ALRT' (id) { ..., { OK, OK, OK, OK }, ... }` is wrong against the Multiverse.r template — the stages array wants three fields per stage `(default-button, visibility, sound-mask)`, not one. wasm-rez asserted out at compile time but with a misleading message. Fixed in #297 for wordpad/notepad/mdpad; the same PR added `scripts/audit-wasm-rez.mjs` so the bug class is caught in CI for every sample on every PR. |
| **Vendored period apps under-size their heap** (mitigated 2026-05-17) | Period apps' default `SIZE -1` hint is small by modern standards (Glypha shipped 1 MB; its full resource fork + GWorlds + sound buffers blew past that at startup, surfacing as `memFullErr` mid-`LoadBufferSounds`). Mitigation is documented in `docs/DEBUGGING-VENDORED-APPS.md`: start at 4 MB pref / 2 MB min; bump if the diagnostic shows `FreeMem` running tight. Not eliminable in source — vendored upstream `.r` is the source of truth, the overlay header is where we set the right hint. |
